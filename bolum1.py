...
- **Nicel (sayısal):** Sayısal ölçümler.
  ...
  - **Sürekli:** Ölçülebilir. Örnek: boy, kilo, süre.
        ...
        query = st.text_input("Bir değişken yazın", placeholder="Örn: aylık gelir", key="b1_query")
        if query:
            q = query.lower()
        ...
        mode = st.radio("Veri tipi", ["Kategorik Veri", "Sayısal Veri"], horizontal=True, key="b1_freq_mode")

        if mode == "Kategorik Veri":
            sample = "Mühendis, Öğretmen, Mühendis, Doktor, Öğretmen, Öğretmen, Avukat"
            raw = st.text_area("Kategorileri virgül ile girin", sample, height=120, key="b1_cat_raw")
            try:
            ...
            raw = st.text_area("Sayıları virgül ile girin", sample, height=120, key="b1_num_raw")
            class_count = st.slider("Sınıf sayısı", 3, 10, 5, key="b1_class_count")
            table_type = st.radio("Tablo tipi", ["Basit frekans tablosu", "Sınıflı frekans tablosu"], horizontal=True, key="b1_table_type")
            try:
        ...
        if graph_type in ["Çubuk Grafik", "Daire Grafik"]:
            ...
            raw = st.text_area("Tam sayıları girin", "12,13,14,15,21,22,22,25,31,34,35,39", height=100, key="b1_stem")
            try:
                data = parse_numeric_input(raw)
            ...
            raw = st.text_area("Zamana göre değerleri girin", "120,132,128,140,150,148,160", height=100, key="b1_time")
            try:
                data = parse_numeric_input(raw)
        ...
        with st.expander("Soru 1 - Kategorik veri için frekans tablosu ve grafik"):
            ...
            if st.button("Soru 1 çözümünü göster", key="b1_sol1"):
        ...
        with st.expander("Soru 2 - Sayısal veri için temel istatistikler"):
            ...
            st.code("4, 6, 8, 10, 12")
            if st.button("Soru 2 çözümünü göster", key="b1_sol2"):
        ...
        with st.expander("Soru 3 - Sınıflı frekans tablosu ve histogram"):
            ...
            if st.button("Soru 3 çözümünü göster", key="b1_sol3"):
        ...
        data_mode = st.radio("Veri yapısı", ["Sayısal Veri", "Kategorik Veri"], horizontal=True, key="b1_lab_mode")
